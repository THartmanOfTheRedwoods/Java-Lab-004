...
The value pf `blimp` is "rattle" when `baffle` gets invoked
...
1. What happens if you invoke a value method and don’t do anything with the result; that is, if you don’t assign it to a variable or use it as part of a larger expression?  
   If you invoke a value method and don't do anything with the result, the program will move on to the next method. Since the value is not assigned to 
a variable the stack diagram won't have that value. However, if the method is public, you'll be able to call that method in the future and assign it a value then. 
2. What happens if you use a void method as part of an expression? For example, try `System.out.println("boo!") + 7;`.  
If you use a void method as part of an expression it will print of a sytem error since it doesn't have a method to call it from. In the example above, the output will print out that the line is not a statement. 
---
